...
void selective_scan_bwd_zohs_kernel(SSMParamsBwd params) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    ddelta_vals[i] += dx * A_val * a + ddelta_u * float(u_vals[i]);
...
void selective_scan_bwd_zohs_launch(SSMParamsBwd &params, cudaStream_t stream) {
            ...
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                    BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                        ...
                        auto kernel = &selective_scan_bwd_zohs_kernel<Ktraits>;
...
void selective_scan_bwd_zohs_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_bwd_zohs_launch<32, 4, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_bwd_zohs_launch<32, 8, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_bwd_zohs_launch<32, 16, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_bwd_zohs_launch<64, 16, input_t, weight_t>(params, stream);
    } else {
        selective_scan_bwd_zohs_launch<128, 16, input_t, weight_t>(params, stream);
    }
}