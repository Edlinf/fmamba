...
void selective_scan_bwd_fohs_kernel(SSMParamsBwd params) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    if (i < kNItems - 1) {
                        thread_data[i] = make_float2(delta_a_exp, !kIsVariableB ? delta_vals[i] * float(u_vals[i]) : .5f * delta_vals[i] * (float(u_vals[i]) + float(u_vals[i+1])) * B_vals[i]);
                    } else {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    float a;
                    if (i < kNItems - 1){
                        du_vals[i] += .5f * dx_B * delta_vals[i];
                        du_vals[i+1] += .5f * dx_B * delta_vals[i];
                        a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : .5f * delta_vals[i] * (float(u_vals[i]) + float(u_vals[i+1])) * B_vals[i]);
                        ddelta_vals[i] += dx * A_val * a + .5f * dx_B * (float(u_vals[i]) + float(u_vals[i+1]));
                    } else {
                        du_vals[i] += dx_B * delta_vals[i];
                        a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                        ddelta_vals[i] += dx * A_val * a + dx_B * float(u_vals[i]);
                    }
                    ...
                    if constexpr (kIsVariableB) { 
                        if (i < kNItems - 1){
                            dB_vals[i] = .5f * dx * delta_vals[i] * (float(u_vals[i]) + float(u_vals[i+1])); 
                        } else {
...
void selective_scan_bwd_fohs_launch(SSMParamsBwd &params, cudaStream_t stream) {
            ...
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                    BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                        ...
                        auto kernel = &selective_scan_bwd_fohs_kernel<Ktraits>;
...
void selective_scan_bwd_fohs_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_bwd_fohs_launch<32, 4, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_bwd_fohs_launch<32, 8, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_bwd_fohs_launch<32, 16, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_bwd_fohs_launch<64, 16, input_t, weight_t>(params, stream);
    } else {
        selective_scan_bwd_fohs_launch<128, 16, input_t, weight_t>(params, stream);
    }
}