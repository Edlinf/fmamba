...
void selective_scan_bwd_zohf_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            // thread_reverse_data <A_t+1, dL/dyt*Ct>
            if constexpr (!kIsComplex) {
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    const float a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : (1.f + .5f * delta_vals[i] * A_val) * delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    ddelta_vals[i] += dx * A_val * a + dx * (1 + delta_vals[i] * A_val) * B_vals[i] * float(u_vals[i]);
                    ...
                    if constexpr (kIsVariableB) { dB_vals[i] = dx * (1.f + .5f * delta_vals[i] * A_val) * delta_vals[i] * float(u_vals[i]); }
                    if constexpr (kIsVariableC) {
...
void selective_scan_bwd_zohf_launch(SSMParamsBwd &params, cudaStream_t stream) {
            ...
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                    BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                        ...
                        auto kernel = &selective_scan_bwd_zohf_kernel<Ktraits>;
...
void selective_scan_bwd_zohf_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_bwd_zohf_launch<32, 4, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_bwd_zohf_launch<32, 8, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_bwd_zohf_launch<32, 16, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_bwd_zohf_launch<64, 16, input_t, weight_t>(params, stream);
    } else {
        selective_scan_bwd_zohf_launch<128, 16, input_t, weight_t>(params, stream);
    }
}