...
void selective_scan_bwd_fohf_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            // thread_reverse_date <A_t+1, dL/dyt*Ct>
            if constexpr (!kIsComplex) {
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    const float delta_a_exp = exp2f(delta_vals[i] * A_scaled);
                    if (i < kNItems - 1){
                        thread_data[i] = make_float2(delta_a_exp, !kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * B_vals[i] * (.5f * (float(u_vals[i]) + float(u_vals[i+1])) + delta_vals[i] * A_val * (float(u_vals[i])/3.f + float(u_vals[i+1])/6.f) ) );
                    } else {
                        thread_data[i] = make_float2(delta_a_exp, !kIsVariableB ? delta_vals[i] * float(u_vals[i]) : (1.f + .5f * delta_vals[i] * A_val) * delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    }
                    if (i == 0) {
                ...
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    const float dx = thread_reverse_data[i].y; //dx = dL/dht
                    const float delta_a = delta_vals[i] * A_val;
                    float a;
                    const float dx_B = dx * B_vals[i];
                    // const float ddelta_u = !kIsVariableB ? dx : dx * B_vals[i];
                    // const float ddelta_u = !kIsVariableB ? dx : dx * (1.f + .5f * delta_vals[i] * A_val) * B_vals[i];
                    
                    if (i < kNItems - 1){
                        du_vals[i] += dx_B * delta_vals[i] * (.5f + delta_a / 3.f);
                        du_vals[i+1] += dx_B * delta_vals[i] * (.5f + delta_a / 6.f);
                        a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * B_vals[i] * (.5f * (float(u_vals[i]) + float(u_vals[i+1])) + delta_a * (float(u_vals[i])/3.f + float(u_vals[i+1])/6.f)));
                        ddelta_vals[i] += dx * A_val * a + dx_B * (.5f * (float(u_vals[i]) + float(u_vals[i+1])) + 2.f * delta_a * (float(u_vals[i])/3.f + float(u_vals[i+1])/6.f));
                        dA_val += dx * delta_vals[i] * (a + delta_vals[i] * B_vals[i] * (float(u_vals[i])/3.f + float(u_vals[i+1])/6.f));
                    } else {
                        du_vals[i] += dx_B * delta_vals[i] * (1.f + .5f * delta_a);
                        a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : (1.f + .5f * delta_a) * delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                        ddelta_vals[i] += dx * A_val * a + dx_B * (1 + delta_a) * float(u_vals[i]);
                        dA_val += dx * delta_vals[i] * (a + .5f * delta_vals[i] * B_vals[i] * float(u_vals[i]));
                    }
                    ...
                    if constexpr (kIsVariableB) { 
                        if (i < kNItems - 1){
                            dB_vals[i] = dx * delta_vals[i] * (.5f * (float(u_vals[i]) + float(u_vals[i+1])) + delta_a * (float(u_vals[i])/3.f + float(u_vals[i+1])/6.f));
                        } else {
                            dB_vals[i] = dx * (1.f + .5f * delta_vals[i] * A_val) * delta_vals[i] * float(u_vals[i]);
                        }
                    }
                    if constexpr (kIsVariableC) {
...
void selective_scan_bwd_fohf_launch(SSMParamsBwd &params, cudaStream_t stream) {
            ...
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                    BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                        ...
                        auto kernel = &selective_scan_bwd_fohf_kernel<Ktraits>;
...
void selective_scan_bwd_fohf_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_bwd_fohf_launch<32, 4, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_bwd_fohf_launch<32, 8, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_bwd_fohf_launch<32, 16, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_bwd_fohf_launch<64, 16, input_t, weight_t>(params, stream);
    } else {
        selective_scan_bwd_fohf_launch<128, 16, input_t, weight_t>(params, stream);
    }
}