...
void selective_scan_fwd_zohs_cuda(SSMParamsBase &params, cudaStream_t stream);

template <typename input_t, typename weight_t>
void selective_scan_fwd_zohf_cuda(SSMParamsBase &params, cudaStream_t stream);

template <typename input_t, typename weight_t>
void selective_scan_fwd_fohs_cuda(SSMParamsBase &params, cudaStream_t stream);

template <typename input_t, typename weight_t>
void selective_scan_fwd_fohf_cuda(SSMParamsBase &params, cudaStream_t stream);

template <typename input_t, typename weight_t>
void selective_scan_bwd_zohs_cuda(SSMParamsBwd &params, cudaStream_t stream);

template <typename input_t, typename weight_t>
void selective_scan_bwd_zohf_cuda(SSMParamsBwd &params, cudaStream_t stream);
...
selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(u.scalar_type(), "selective_scan_fwd", [&] {
        DISPATCH_WTYPE_FLOAT_AND_COMPLEX(A.scalar_type(), "selective_scan_fwd", [&] {
            ...
            if(mode == 0) {
                selective_scan_fwd_zohs_cuda<input_t, weight_t>(params, stream);
            }else if(mode == 1){
                selective_scan_fwd_zohf_cuda<input_t, weight_t>(params, stream);
            }else if(mode == 2){
                selective_scan_fwd_fohs_cuda<input_t, weight_t>(params, stream);
            }else if(mode == 3){
                selective_scan_fwd_fohf_cuda<input_t, weight_t>(params, stream);
            }
        });
    });
...
selective_scan_bwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(u.scalar_type(), "selective_scan_bwd", [&] {
        DISPATCH_WTYPE_FLOAT_AND_COMPLEX(A.scalar_type(), "selective_scan_bwd", [&] {
            ...
            if(mode == 0) {
                selective_scan_bwd_zohs_cuda<input_t, weight_t>(params, stream);
            }else if(mode == 1){
                selective_scan_bwd_zohf_cuda<input_t, weight_t>(params, stream);
            }else if(mode == 2){
                selective_scan_bwd_fohs_cuda<input_t, weight_t>(params, stream);
            }else if(mode == 3){
                selective_scan_bwd_fohf_cuda<input_t, weight_t>(params, stream);
            }
        });
    });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    // m.def("fwd", &selective_scan_fwd, "Selective scan forward", pybind11::arg("mode") = 0);
    m.def("fwd", &selective_scan_fwd, "Selective scan forward");
    // m.def("bwd", &selective_scan_bwd, "Selective scan backward", pybind11::arg("mode") = 0);
